...
def first_bad_version(n, start = 1, finish = n)
    ...
    if is_bad_version(mid) 
        ...
        return first_bad_version(n, mid, finish)
    end
end

def first_bad_version(n, start = 0, finish = n)
    mid = (start+finish)/2
    if is_bad_version(mid) 
        return  0 if mid == 0
        return mid if !is_bad_version(mid-1)
        return first_bad_version(n, start, mid-1)
    else
        return first_bad_version(n, mid+1, finish)
    end
end
